...
def detect_armed_person(rtsp_url, camera_id, site_id, display_width, display_height, type, co_ordinate, stop_event):
    ...
    try:
        ...
        confidence_threshold = 0.5  # Set an appropriate threshold (adjust based on performance)

        # Non-maximum suppression (NMS) threshold
        nms_threshold = 0.5  # Adjust to control the level of overlapping boxes (0.5 is a common default)

        cap = cv2.VideoCapture(rtsp_url)
        if not cap.isOpened():
            raise ArmError(f"Failed to open Camera: {camera_id}")

        window_name = f'Armed Person Detection - Camera {camera_id}'
        ...
        while not stop_event.is_set():
            ...
            for info in results:
                ...
                for box in parameters:
                    ...
                    confidence = float(box.conf[0])  # Get confidence of the detection

                    current_time = time.time()

                    if class_detect in armed_classes and confidence > confidence_threshold:
...
def armed_detection_start(task):
    """
